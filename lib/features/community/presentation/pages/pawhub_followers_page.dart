...
class PawHubFollowersPage extends ConsumerStatefulWidget {
  const PawHubFollowersPage({required this.petId, super.key});

  final int petId;

  @override
  ConsumerState<PawHubFollowersPage> createState() =>
      _PawHubFollowersPageState();
}

class _PawHubFollowersPageState extends ConsumerState<PawHubFollowersPage> {
  @override
  Widget build(BuildContext context) {
    final followersAsync = ref.watch(followersProvider(widget.petId));
    ...
    return Scaffold(
      ...
      body: followersAsync.when(
        ...
        error: (_, _) => _errorState(
          () => ref.invalidate(followersProvider(widget.petId)),
        ),
        data: (page) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        if (i == page.pets.length - 1 && page.cursor.hasMore) {
          ref.read(followersProvider(widget.petId).notifier).loadMore();
        }
        return _PetTile(
          pet: pet,
          petId: widget.petId,
